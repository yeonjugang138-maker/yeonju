...
class DB_b:
    ...
    def fetch_origin_by_code(self, origin_code):

        sql = "SELECT origin_code, origin_name FROM origin WHERE origin_code = %s"
    ...
    # [삭제됨] 원래 파일 맨 아래에 아래와 같은 메서드가 붙어 있었음:
    #
    #   def save_fruit(self):
    #       item_name = self.fruit_name.text()
    #       origin_code = self.comboBox.currentData()
    #       self.db.insert_item(item_name, origin_code)
    #
    # DB_b는 UI 클래스가 아니라서 self.fruit_name, self.comboBox, self.db가
    # 존재하지 않음. fruit_add.py에 있던 코드를 잘못 복붙한 것으로 보여 삭제함
    # (실제 저장 로직은 fruit_add.py의 FruitAdd.save_fruit에 이미 있음).
