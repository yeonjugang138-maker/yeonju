"""
-- ============================================================
-- 과일 가게 재고관리 프로그램 - DB 스키마 (최종본)
-- ============================================================
-- 개발 과정에서 여러 번 ALTER TABLE로 컬럼을 추가해왔는데,
-- 이 파일은 그 이력을 다 반영한 "최종 결과"만 깔끔하게 정리한 것입니다.
-- 새로 DB를 만드는 경우 이 파일 하나만 실행하면 됩니다.
-- ============================================================
...
-- origin : 원산지
-- ------------------------------------------------------------
-- origin_code는 지역별 고정 코드 (예: 제주 -> jj). 화면에 보이는
-- "jj-011" 같은 배치 코드는 이 값에 화면 표시용 번호를 붙인 것일 뿐,
-- DB에는 origin_code 원본 값만 저장된다.
CREATE TABLE origin (
...
-- items : 과일 (품목)
-- ------------------------------------------------------------
-- 과일 이름이 같아도 원산지가 다르면 별개의 행(item_id)으로 관리한다.
-- (예: "감귤"-제주 와 "감귤"-영동은 서로 다른 item_id)
CREATE TABLE items (
...
-- ============================================================
-- 참고: 재고 수량이 어긋났을 때 다시 맞추는 SQL
-- (items.quantity가 실제 배치 합계와 다를 경우 사용)
-- ============================================================
-- UPDATE items i
-- SET quantity = (
--     SELECT COALESCE(SUM(remaining_quantity), 0)
--     FROM purchase p
--     WHERE p.item_id = i.item_id AND p.is_deleted = 0
-- )
-- WHERE item_id > 0;
